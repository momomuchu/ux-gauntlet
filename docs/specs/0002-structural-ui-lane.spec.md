...
- [CRITICAL][BLOCKS:high] The operator-declared expected-main-content anchor is verified as found on the page, inside the single main landmark, and the largest visible text block by rendered bounding-box area — measured over the candidate set `main, article, section, div, p, h1`, excluding from the comparison pool only those candidates that are ancestors of the declared anchor itself (never the anchor, never its own descendants) and any candidate whose computed style resolves `display:none` / `visibility:hidden` / zero bounding-box area, ties broken by DOM order. Containment inside `<main>` does not imply equal-or-greater prominence, and the anchor is never disqualified by a candidate it contains (CR2-M6 / CR3-B3). Requirement ID: S12.
...
- [MEDIUM][BLOCKS:high] For the one live cross-namespace equivalence pair (landmark-one-main axe rule ID <-> cannot-evaluate-ambiguous-main finding code, both pinned to the `:root` target-element selector), when the axe finding and its paired non-axe finding share that `:root` selector, the non-axe finding is emitted and the axe finding is suppressed — so one physical defect flagged by both a whole-document axe rule and a custom check is not double-counted with contradictory severities. The tabindex/positive-tabindex pairing is intentionally dropped: S1 disables the axe `tabindex` rule (CR2-B13), so no axe tabindex finding can ever reach this suppression (CR3-2). Requirement ID: S55.
...
- Requirements: `.swe-spec-0002/requirements.txt` (64 lines: 56 functional S1-S56, 8 nonfunctional SN1-SN8; req-lint 64/64 PASS — see `.swe-spec-0002/lint-result.txt`. CHALLENGE-ROUND-1 appended S35-S46 + SN7; CHALLENGE-ROUND-2 appended S47-S55 + SN8 and edited S1/S4/S12/S14/S15/S39/S44 in place; CHALLENGE-ROUND-3 appended S56 (CI-block for cannot-evaluate-ambiguous-main, closing the CR3-B1 fail-closed inversion), removed the dead tabindex branch from S55, and edited S1/S12/S21/S22/S24/S42/S44/S45/S49/S55/SN8 in place — tracing every changed line inline as `# CR3-<defect-id>` with no renumbering).
