...
(network-idle + `document.fonts.ready`) has completed, the same DOM snapshot + the same pinned
...
- **The determinism invariant (S21/S42) is asserted as a gate-checked contract over a recorded finding-id set (axe violations + axe incomplete results + non-axe custom checks), not proven by two live consecutive browser runs.** Live double-run proof is deferred to the same v2 suite. S21 is scoped to apply only once the S1 settle precondition (network-idle + `document.fonts.ready`) has completed, because color-contrast can otherwise resolve as violation on one run and incomplete on another purely from paint timing (CR1-M2); the pinned browser binary version (SN7) closes the remaining render-engine variance (CR1-M12).
...
| D5b | Browser-version-pin policy | **Pin the Playwright browser binary version, disclose it in report metadata (SN7); a bump requires a decision record (S43)** | axe measures contrast against the DOM the pinned Chromium renders; a font/rendering change in an unpinned browser can flip a borderline 4.499:1 finding and trip the CI gate with no decision-record trail — S32 pinned only axe-core, leaving the render engine unpinned (CR1-M12). Reversible with the same trail as D5 |
| D6 | Schema family | **Same findings-schema family as SPEC 0001, discriminated by a mandatory `lane:"structural"` field; one CI gate handles both** | one schema family + one gate script keeps the two lanes reportable side by side while the `lane` discriminator makes accidental fusion structurally impossible (brief §6.6); the cross-reference join uses the field name `target_element_identifier` that actually exists in the 0001 schema (CR1-B4). Reversible: founder may split into a fully separate schema/gate |
...
- Requirements: `.swe-spec-0002/requirements.txt` (53 lines: 46 functional S1-S46, 7 nonfunctional SN1-SN7; req-lint 53/53 PASS — see `.swe-spec-0002/lint-result.txt`. CHALLENGE-ROUND-1 appended S35-S46 + SN7 and traced every changed line inline as `# CR1-<defect-id>`).
