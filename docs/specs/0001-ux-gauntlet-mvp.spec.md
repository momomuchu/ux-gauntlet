...
- [CRITICAL][BLOCKS:high] Per-step 4-question cognitive-walkthrough record (right goal? element noticed? mapping understood? feedback seen?) before advancing. Requirement ID: F9.
- [CRITICAL][BLOCKS:high] Friction accounting: one named friction instance per action beyond the happy path; exactly one criterion tag from the configured heuristic set (data-loaded; default: Nielsen 10); 0–4 severity from frequency-impact-persistence. Requirement ID: F10, F11, F12, F28.
...
- [CRITICAL][BLOCKS:low] Every report carries the validity-envelope disclosure; never emits WTP estimates or population-percentage extrapolations. Requirement ID: F20, F21, F22.
...
| D4 | Standardized-flow handling | Operator allowlist (no auto-detect) | auto-detection is unproven; allowlist is deterministic |
