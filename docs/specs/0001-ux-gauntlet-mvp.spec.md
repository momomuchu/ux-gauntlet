...
- [CRITICAL][BLOCKS:critical] Evidence secret redaction: before any evidence artifact is written to disk, the evidence capture module redacts credential-or-payment-data pattern matches from both a screenshot-type evidence entry's captured_text sidecar field and DOM snippets. Requirement ID: F43, F44, F102.
...
- [HIGH][BLOCKS:low] A first-time operator reaches a started crawl against a reachable localhost target using only the shipped example tasks file, the shipped default denylist, and `--i-own-this-target`, in one CLI invocation with zero failed prior attempts. Requirement ID: N8.
...
### MEDIUM items
- [MEDIUM][BLOCKS:none] CI mode: headless, non-interactive; exits nonzero on a new severity-4 finding vs the committed baseline, and independently exits nonzero whenever the run's run_status is BLOCKED regardless of severity-4 content. Requirement ID: F26, F101, N4.
- [MEDIUM][BLOCKS:none] Operator-allowlisted standardized flows (login, stock checkout) labeled lower-confidence in the report (cognitive walkthrough is weaker there); the lower-confidence label is cosmetic only and does not suppress the CI gate on a severity-4 finding. Requirement ID: F27, F121.
- [MEDIUM][BLOCKS:none] 3-persona × 5-step run completes ≤45 min on one machine (derivation: personas × the 50-action runner cap × per-action budget, see requirements.txt comment); localhost targets need no third-party service beyond the agent runtime. Requirement ID: N5, N6.
...
- [MEDIUM][BLOCKS:high] The findings schema assigns each finding a finding_id computed as a deterministic hash of exactly the (heuristic_tag, normalized_step_id, target_element_identifier) tuple — the same tuple as the F45 dedup key, never persona role, never ledger arrival order (see D12); the target_element_identifier itself is computed via the F45/F103 priority-ordered selector fallback. Requirement ID: F92, F103.
...
- Requirements: `.swe-spec/requirements.txt` (133 lines: 125 functional, 8 nonfunctional; req-lint 133/133 PASS — see `.swe-spec/lint-result.txt`).
