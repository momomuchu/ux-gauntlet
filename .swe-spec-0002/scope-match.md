...
Scope: 53 requirements (46 functional S1-S46, 7 nonfunctional SN1-SN7 after CHALLENGE-ROUND-1
appended S35-S46 + SN7). Build surface reuses the existing SPEC 0001 machinery rather than standing
up a new subsystem:
...
- 1 ADR `docs/adr/0005-axe-core-version-pin.md` (D5/S32 governance, extended to the pinned Playwright
  browser binary per S43/SN7/D5b — one version-pin ADR covers both engines).
- fixtures + the RED test `test/acceptance-0002.test.mjs` (CHALLENGE-ROUND-1 added ~16 fixtures and
  exit-code assertions on every -bad.json case, so the ATDD lock bites on gate exit codes, not stdout
  keywords).
